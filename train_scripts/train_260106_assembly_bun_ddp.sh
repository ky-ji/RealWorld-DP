...
echo "配置文件: train_assembly_bun_ddp.yaml"
echo ""
...
echo "命令: torchrun --nproc_per_node=$NUM_GPUS --master_port=29500 train_ddp.py --config-name=train_assembly_bun_ddp"
echo ""
...
torchrun \
    ...
    --master_port=29500 \
    train_ddp.py \
    --config-name=train_assembly_bun_ddp
