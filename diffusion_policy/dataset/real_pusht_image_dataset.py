...
class RealPushTImageDataset(BaseImageDataset):
    def __init__(self,
        ...
        if delta_action:
            ...
            # support positions only at this time (extended to support 7D for robot tasks)
            # For 7D: [x, y, z, rx, ry, rz, gripper]
            # We convert position and rotation to delta, but keep gripper as absolute
            ...
            for i in range(len(episode_ends)):
                ...
                if actions.shape[1] <= 3:
                    # 2D or 3D position only
                    actions_diff[start+1:end] = np.diff(actions[start:end], axis=0)
                elif actions.shape[1] == 7:
                    # 7D: [x, y, z, rx, ry, rz, gripper]
                    # Convert position (xyz) and rotation (rxryrz) to delta
                    actions_diff[start+1:end, :6] = np.diff(actions[start:end, :6], axis=0)
                    # Keep gripper as absolute value
                    actions_diff[start:end, 6] = actions[start:end, 6]
                else:
