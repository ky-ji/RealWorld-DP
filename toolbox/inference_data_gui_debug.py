...
class InferenceGUI:
    ...
    def plot_replay_frame(self, step_idx):
        ...
        with c1:
            ...
            # 显示关键时延指标
            if timing:
                t_transport = timing.get('transport_latency', 0) * 1000
                t_infer = timing.get('inference_latency', 0) * 1000
                total = timing.get('total_latency', 0) * 1000
    ...
    def plot_latency_analysis(self):
        if not self.timings or 'inference_latency' not in self.timings[0]:
            st.warning("当前日志不包含详细时延数据")
            return

        steps = range(len(self.timings))
        trans_lats = [t.get('transport_latency', 0)*1000 for t in self.timings]
        infer_lats = [t.get('inference_latency', 0)*1000 for t in self.timings]
        total_lats = [t.get('total_latency', 0)*1000 for t in self.timings]
...
log_dir = Path(__file__).parent.parent / "server" / "log"
